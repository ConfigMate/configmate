...
func (a *AnalyzerImpl) AnalyzeConfigFiles(files map[string]*parsers.Node, rules []Rule) (res []Result, err error) {
	...
	for _, rule := range rules {
		...
		allTokenLocations := make([]TokenLocationWithFileAlias, 0) // list of token locations for the check
			...
			case File:
				...
				if value, err := files[fArg.alias].Get(fArg.path); err != nil { // Get value from file
					res = append(res, Result{
						...
						TokenList:     []TokenLocationWithFileAlias{makeTOFTokenLocation(fArg.alias)},
					})
					errors = true
					...
					res = append(res, Result{
						...
						TokenList:     []TokenLocationWithFileAlias{makeValueTokenLocation(fArg.alias, value)},
					})
					errors = true
...
func (a *AnalyzerImpl) AnalyzeRuleBook(rulebookFileTree *parsers.Node) (res []Result) {
	...
	if nameNode, err := rulebookFileTree.Get("name"); err != nil {
		res = append(res, Result{
			...
			TokenList:     []TokenLocationWithFileAlias{makeTOFTokenLocation(rulebookFileAlias)},
		})
	} else {
		if nameNode.Type != parsers.String {
			res = append(res, Result{
				...
				TokenList:     []TokenLocationWithFileAlias{makeValueTokenLocation(rulebookFileAlias, nameNode)},
			})
	...
	if description, err := rulebookFileTree.Get("description"); err != nil {
		...
		if description.Type != parsers.String {
			res = append(res, Result{
				...
				TokenList:     []TokenLocationWithFileAlias{makeValueTokenLocation(rulebookFileAlias, description)},
			})
	...
	if files, err := rulebookFileTree.Get("files"); err != nil {
		...
		if files.Type != parsers.Object {
			res = append(res, Result{
				...
				TokenList:     []TokenLocationWithFileAlias{makeValueTokenLocation(rulebookFileAlias, files)},
			})
			...
			for key, value := range files.Value.(map[string]*parsers.Node) {
				...
				if key == "" {
					res = append(res, Result{
						...
						TokenList:     []TokenLocationWithFileAlias{makeNameTokenLocation(rulebookFileAlias, value)},
					})
				}
				...
				if value.Type != parsers.String {
					res = append(res, Result{
						...
						TokenList:     []TokenLocationWithFileAlias{makeValueTokenLocation(rulebookFileAlias, value)},
					})
	...
	if rules, err := rulebookFileTree.Get("rules"); err != nil {
		...
		if rules.Type != parsers.Array {
			res = append(res, Result{
				...
				TokenList:     []TokenLocationWithFileAlias{makeValueTokenLocation(rulebookFileAlias, rules)},
			})
			...
			for _, rule := range rules.Value.([]*parsers.Node) {
				...
				if rule.Type != parsers.Object {
					res = append(res, Result{
						...
						TokenList:     []TokenLocationWithFileAlias{makeValueTokenLocation(rulebookFileAlias, rule)},
					})
				} else {
					...
					if checkName, err := rule.Get("check"); err != nil {
						...
						if checkName.Type != parsers.String {
							res = append(res, Result{
								...
								TokenList:     []TokenLocationWithFileAlias{makeValueTokenLocation(rulebookFileAlias, checkName)},
							})
							...
							if args, err := rule.Get("args"); err != nil {
								...
								if args.Type != parsers.Array {
									res = append(res, Result{
										...
										TokenList:     []TokenLocationWithFileAlias{makeValueTokenLocation(rulebookFileAlias, args)},
									})
									...
									for i, argNode := range args.Value.([]*parsers.Node) {
										if _, ok := argNode.Value.(string); !ok { // Ensure value is represented as a string
											res = append(res, Result{
												...
												TokenList:     []TokenLocationWithFileAlias{makeValueTokenLocation(rulebookFileAlias, argNode)},
											})
